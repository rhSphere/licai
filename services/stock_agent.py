...
_SYSTEM = (
    ...
    "  · 消息面(新闻/政策/社媒情绪/研报)不休市, 周末照常发酵、甚至更密集(没盘干扰, 消息集中酝酿下周)。所以**周末更要主动 web_search**, 带日期去找周六周日及最近几天的新消息——这正是看'下周开盘前在酝酿什么'的黄金窗口。查到周末/近几天日期的新催化, 就是真'这两天/周末在发酵', 大胆说。\n"
    "  · 行情数据(价格/资金流/涨跌/情绪温度)周末冻结在周五收盘快照: get_market_sentiment/get_sector_momentum/get_hot_concepts 周末返回的是上一交易日(周五)的读数, 引用时点明'周五收盘快照', 别说成'今天的资金还热'。\n"
    "  · 每个事件标真实日期, 把'题材背景(旧, 标真实月份如1月出口管制/2月钨价+37%)'和'近几天新催化(标具体日期)'分开讲, 别把几个月前的旧事堆成'周末在发酵'。\n"
    "  · 真搜过了近几天确实没新进展, 才说'近两天没搜到新消息, 以下是更早的背景脉络'——是先搜后说, 不是用'周末休市'当借口不搜。\n"
